...
|-------------------------------|-----------------------------------------------|------------------------------------------------------------------------------------------------------------------------|
| `API_PORT`                    | `8080` when unset                             | HTTP port. When unset, API first tries 8080, then automatically discovers the next available port (logging the chosen port). Set `API_PORT` to force a specific port (process will fail if that port is in use). **POST /chat** requires Temporal and the Worker to be running; the API returns 503 until then. |
